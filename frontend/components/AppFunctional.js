...
export default function AppFunctional(props) {
  ...
  function getNextIndex(direction) {
    ...
    if(direction === 'left' && plot[index][1] > 1) return [(index - 1), true];
    if(direction === 'up' && plot[index][0] > 1) return [(index - 3), true];
    if(direction === 'down' && plot[index][0] < 3) return [(index + 3), true];
    if(direction === 'right' && plot[index][1] < 3) return [(index + 1), true];
    return [index, false];
  }
  ...
  function move(evt) {
    ...
    const [newSpot, isPossible] = getNextIndex(evt.target.id);
